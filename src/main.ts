...
window.addEventListener("DOMContentLoaded", () => {                     // This section sets up initial event listeners
  window.focus();                                                       // and focuses the window when the DOM is ready.
                                                                        // 
  document.querySelector("#test")?.addEventListener("click", () => {    // This logs a message when a button with
    console.log("Button Clicked");                                      // the ID "test" is clicked.
  });                                                                   // 
                                                                        // 
  setTimeout(() => {                                                    // This part sets logging and focusing
    ...
    console.log("Focused element:", document.activeElement);            // the program remains ready.
                                                                        ...
                                                                        // 
  window.addEventListener("click", () => {                              // This refocuses the window upon any
...
async function dispatch(action: string, payload?: string) {    // This relays an action to the Rust
  state = await invoke<AttoSnapshot>("atto_action", {          // backend via Tauri. It calls the
...
window.addEventListener("keydown", async (e) => {        // This section does keyboard event
  ...
  switch (e.key) {                                       // Switch cases are then used to respond to
    ...
    default:                                             // This is the default case
      if (e.key.length === 1) {                          // which simply sends the order
...
function render() {                                                // This simply updates the DOM to reflect  
  ...
  state.buffer.forEach((line, y) => {                              // Creates a buffer for each line 
    const lineDiv = document.createElement("div");                 // using whitespace: "pre"
    lineDiv.style.whiteSpace = "pre";                              // to preserve spaces.

    if (y === state.cursor_y) {                                                // If the line has the cursor on it:
  ...
  blinkTimeout = window.setTimeout(() => {                                     // After 500ms, it switches the state 
